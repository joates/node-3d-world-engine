  ...
  return (function() {
    ...
    db_get_tile(key, function(err, tile_from_db) {
      if (! tile_from_db) {
        ...
        create_map_tile(x, y, function(err, map_tiles) {
          if (err) cb(err)
...
function create_map_tile(x, y, cb) {
  ...
  weld_edges.call(tile, function(err, map_tiles) {
...
function weld_edges(cb) {
  ...
  , function(err, db_tiles) {
    ...
    // add newly created map_tile to modified store
    map_tiles_modified.push(tile)
    ...
    db_tiles.forEach(function(db_tile, db_tile_id) {
      ...
      // add updated db_tile to modified store
      map_tiles_modified.push(db_tile)
