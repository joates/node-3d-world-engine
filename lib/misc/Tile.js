  ...
  if (grid_x !== -1 && grid_y !== -1) {
    repair_regions.call(tile, function(err, modified_tiles) {
      ...
      modified_tiles.forEach(function(mod_tile, mod_tile_id) {
        ...
        //if (mod_tile_id === 0) mod_tile.mesh_data.push(require('./Geometry').outline(tile))
      })
...
function repair_regions(cb) {
  ...
  build_quad_tile(tile, function(err, quad_tile, all_tiles) {
    ...
    all_tiles.forEach(function(map_tile, map_tile_id) {
      ...
      // avoid re-welding (unless map tile is in the South-West position)
      if ( map_tile.flags.welded_edges & o_o.edge[opts.edges_to_repair[0]]
        && map_tile.flags.welded_edges & o_o.edge[opts.edges_to_repair[1]]
        && map_tile_id !== 2 ) return
      ...
      map_tile.regions.forEach(function(region, region_id) {
        ...
        if (is_boundary && (! is_welded)) {
          ...
          if (opts.edges_to_repair.indexOf(edge_id) !== -1) {
            ...
            all_tiles[map_tile_id].regions[region_id] = encode_welded_boundary_neighbors(region, region_id, map_tile_id, all_tiles, weld)
          }
        }

        else if (! is_boundary)
          all_tiles[map_tile_id].regions[region_id] = encode_interior_neighbors(region, map_tile.key)
      })

      // update the welded edge flags
      if (! (map_tile.flags.welded_edges & o_o.edge[opts.edges_to_repair[0]]))
        all_tiles[map_tile_id].flags.welded_edges += o_o.edge[opts.edges_to_repair[0]]
      if (! (map_tile.flags.welded_edges & o_o.edge[opts.edges_to_repair[1]]))
        all_tiles[map_tile_id].flags.welded_edges += o_o.edge[opts.edges_to_repair[1]]

      // store modified map tiles
      if ( map_tile_id !== 0
        || (all_tiles[map_tile_id].flags.welded_edges === 15 && map_tile_id === 2)) {
    ...
    all_tiles = encode_neighboring_edges(all_tiles)

    cb(null, all_tiles)
  })
}

function encode_neighboring_edges(all_tiles) {
  var all_keys = all_tiles.map(function(tile) { return tile.key })
          ...
          if (typeof neighbor === 'string') {
            ...
            if (target_tile_id !== -1) {
              var target_region_id = parseInt(neighbor.substr(12))
                , neighbor_edge_id = get_delaunay_neighbors_edge_id(all_tiles[target_tile_id], orig_tile_key, orig_region_id, target_region_id)
...
function get_delaunay_neighbors_edge_id(map_tile, orig_tile_key, orig_region_id, neighbor_region_id) {
  for (var i = 0, il = map_tile.regions[neighbor_region_id].edges.length; i < il; i++) {
    var id = map_tile.regions[neighbor_region_id].edges[i].delaunay_neighbor
    if (typeof id === 'string') {
...
function encode_welded_boundary_neighbors(region, region_id, tile_id, all_tiles, weld) {
  var target_region_id = weld.map[tile_id].orig_quad[region_id]
    , encoded_neighbors = {}
    , edge_map = {}
  ...
  weld.regions[target_region_id].neighbors.forEach(function(target_neighbor_id) {
    ...
    if (pos.x > tile_size && pos.z < tile_size) {
      if (! encoded_neighbors[all_tiles[0].key]) encoded_neighbors[all_tiles[0].key] = []
      encoded_neighbors[all_tiles[0].key].push(weld.map[0].quad_orig[target_neighbor_id])
      ...
      if (! encoded_neighbors[all_tiles[1].key]) encoded_neighbors[all_tiles[1].key] = []
      encoded_neighbors[all_tiles[1].key].push(weld.map[1].quad_orig[target_neighbor_id])
      if (typeof delaunay_neighbor === 'number')
        region.edges[edge_map[target_neighbor_id]].delaunay_neighbor = all_tiles[1].key +'_'+ weld.map[1].quad_orig[target_neighbor_id]
    }

    else if (pos.x < tile_size && pos.z > tile_size) {
      if (! encoded_neighbors[all_tiles[2].key]) encoded_neighbors[all_tiles[2].key] = []
      encoded_neighbors[all_tiles[2].key].push(weld.map[2].quad_orig[target_neighbor_id])
      if (typeof delaunay_neighbor === 'number')
        region.edges[edge_map[target_neighbor_id]].delaunay_neighbor = all_tiles[2].key +'_'+ weld.map[2].quad_orig[target_neighbor_id]
    }

    else if (pos.x < tile_size && pos.z < tile_size) {
      if (! encoded_neighbors[all_tiles[3].key]) encoded_neighbors[all_tiles[3].key] = []
      encoded_neighbors[all_tiles[3].key].push(weld.map[3].quad_orig[target_neighbor_id])
      if (typeof delaunay_neighbor === 'number')
        region.edges[edge_map[target_neighbor_id]].delaunay_neighbor = all_tiles[3].key +'_'+ weld.map[3].quad_orig[target_neighbor_id]
    }
  })
  region.neighbors = deep_copy(encoded_neighbors)
  return region
}

function encode_interior_neighbors(region, tile_key) {
  if (Array.isArray(region.neighbors)) {
    var neighbors = {}
    neighbors[tile_key] = region.neighbors
    region.neighbors = neighbors

    region.edges.forEach(function(edge, edge_id) {
      if (typeof edge.delaunay_neighbor === 'number')
        edge.delaunay_neighbor = tile_key +'_'+ edge.delaunay_neighbor
    })
  }
  return region
}
