...
function create_region(voronoi_cell, rotation) {
  return (function() {
    var site = voronoi_cell.site
      ...
      , rotation_center = { x: tile_size*0.5, y: tile_size*0.5 }
      , pos = site
      , verts = {}      // Note: starts as an object because need to filter out duplicate vertices
      , edges = []
      , neighbors = []

    if (rotation && rotation.angle) pos = apply_rotation(new THREE.Vector2(site.x, site.y), rotation.ct, rotation.st)
    ...
    voronoi_cell.halfedges.forEach(function(half_edge) {
      ...
      var key1 = ''+ v1.x +','+ v1.y
      if (rotation&& rotation.angle) pos = apply_rotation(v1, rotation.ct, rotation.st)
      else pos = v1
      verts[key1] = { position: { x: pos.x, y: 0, z: pos.y } }

      var key2 = ''+ v2.x +','+ v2.y
      if (rotation && rotation.angle) pos = apply_rotation(v2, rotation.ct, rotation.st)
      else pos = v2
...
function apply_rotation(pos, ct, st) {
  var nx = (ct * (pos.x - rotation_center.x)) + (st * (pos.y - rotation_center.y)) + rotation_center.x
    , ny = (ct * (pos.y - rotation_center.y)) - (st * (pos.x - rotation_center.x)) + rotation_center.y
  return { x: nx, y: ny }
}
